...
<script type="text/javascript">
    $(document).ready(function() {
        $( "#datacontent_filter_wtc" ).load('<?php echo $ajax_lists_filter_wtc; ?>', function(){
            console.log('hai');
        });

        $( "#datacontent_filter_wl" ).load('<?php echo $ajax_lists_filter_wl; ?>', function(){
            console.log('hai');
        });
        ...
        $('#btn_preview').click(function(e) {
            e.preventDefault();
            var param = {
                'url_ajax_action'   : '<?php echo $ajax_lists_filter_wtc; ?>',
                'parameter'         : $('#form_datacontent_filter').serialize(),
                'data_type'         : 'html',
                'callback'          : function(data) {
                    $('#datacontent_filter_wtc').empty().append(data);
                }
            };
            MYAPP.doAjax.process(param.url_ajax_action, param.parameter, param.callback, param.data_type);

            var param = {
                'url_ajax_action'   : '<?php echo $ajax_lists_filter_wl; ?>',
                'parameter'         : $('#form_datacontent_filter').serialize(),
                'data_type'         : 'html',
                'callback'          : function(data) {
                    $('#datacontent_filter_wl').empty().append(data);
                }
            };
            MYAPP.doAjax.process(param.url_ajax_action, param.parameter, param.callback, param.data_type);
        });
...
<div id="panel_list">
    <div class="panel">
        <div class="panel-body">
            <form method="post" class="form-horizontal bordered-row" id="form_datacontent_filter">
                <div class="row">
                    <div class="col-sm-10">
                        <div class="form-group grouphidden" id="summary_user">
    ...
    <div class="panel">
        <div class="panel-body">
            ...
            <div id="datacontent_filter_wtc" class="example-box-wrapper"></div>
        </div>
    </div>
    <div class="panel">
        <div class="panel-body">
            ...
            <div id="datacontent_filter_wl" class="example-box-wrapper"></div>
        </div>
    </div>
</div>