...
class Visitor extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'report_visitor/visitor/';
	private $_table_name 		= 'transaksi';
	private $_table_field_pref 	= 'trans_';
	private $_table_pk 			= 'trans_id_transaksi';
	private $_model_crud 		= 'reportvisitor_model';

	private $_page_title 		= 'Antrian : Admin Laporan Visitor';
	private $_page_content_info	= array(
		'title' => 'Data Admin Laporan Visitor',
		'desc' 	=> 'List laporan Visitor',
	);
		...
		$column_list = array(
            ...
            array(
                ...
                'result_format'			=> function( $d, $row ) {
                    $tipe_antrian = '-';
                    if($row['id_tipe'] == 'UM') $tipe_antrian = 'UMUM';
                    if($row['id_tipe'] == 'GR') $tipe_antrian = 'GURU';
                    return $tipe_antrian;
                },
            ),
            array(
                'title_header_column' 	=> 'Jenis Antrian',
                'field_name' 			=> 'jenis_antrian',
                'no_order'				=> 6,
                'result_format'			=> function( $d, $row ) {
                    $tipe_antrian = '-';
                    if($row['jenis_antrian'] == 'pers') $tipe_antrian = 'Personal';
                    if($row['jenis_antrian'] == 'kol') $tipe_antrian = 'Kolektif';
                    return $tipe_antrian;
                },
            ),
            array(
                'title_header_column' 	=> 'Permasalahan',
                'field_name' 			=> 'permasalahan',
                'no_order'				=> 7,
            ),
            array(
	...
	function lists_ajax_filter() {
		...
		$this->_data['id_tipe'] 		= $this->input->post('id_tipe');
		$this->_data['jenis_antrian'] 		= $this->input->post('jenis_antrian');
	...
	function lists_ajax() {
		...
		$table 		.= ' JOIN ' . $this->db->dbprefix . 'visitor_online ON (id = trans_id_visitor_online) ';
	...
	function page_export_text() {
        ...
        if(!empty($_GET['id_tipe'])) {
            $where[] = 'id_tipe = "' . $_GET['id_tipe']. '"';
        }

        if(!empty($_GET['jenis_antrian'])) {
            $where[] = 'jenis_antrian = "' . $_GET['jenis_antrian'].'"';
        }
		...
		$this->db->select($this->_table_name . '.*, anf_visitor_online.*, lokets_name, lay_nama_layanan, DATE_FORMAT(trans_tanggal_transaksi, "%d-%m-%Y") as own_tanggal, CONCAT(trans_no_ticket_awal, "", trans_no_ticket) as no_ticket, "-" as waktu_tunggu, "-" as waktu_layanan, admusr_username', false);
		$this->db->from($this->_table_name);
		$this->db->join('visitor_online', 'id = trans_id_visitor_online');
		...
		$this->template->set('title', 'Antrian : Export Text Visitor');
		$this->template->set('assets', $this->_data['assets']);
		$this->template->load('template_export/export_text', 'lists_exporttext', $this->_data);
	}

	function page_export_excel() {
		$filename = 'antrian_visitor_' . str_replace('-', '', date('Y-m-d')) . '.xls';
		...
		$this->template->set('title', 'Antrian : Export Excel Visitor');
		$this->template->set('assets', $this->_data['assets']);
