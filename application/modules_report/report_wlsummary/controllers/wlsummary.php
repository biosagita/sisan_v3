...
class wlsummary extends MY_Admin {
	...
	private $_page_title 		= 'Antrian : Admin Laporan Waktu Layanan Summary';
	private $_page_content_info	= array(
		'title' => 'Data Admin Waktu Layanan Summary',
		'desc' 	=> 'List laporan waktu layanan summary',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				...
				'field_name' 			=> 'SEC_TO_TIME(AVG(TIME_TO_SEC(IF(TIMEDIFF(  trans_waktu_finish,  trans_waktu_panggil ) > 0, TIMEDIFF(  trans_waktu_finish,  trans_waktu_panggil ), 0)))) as rata_rata',
				'alias'					=> 'rata_rata',
				'ignore_search'			=> true,
				'no_order'				=> 2,
			),
	...
	function lists_ajax_filter_wl() {
		...
		$this->load->view('lists_filter_wl', $this->_data);
	}
	...
	function lists_ajax($type_summary = '', $value_summary = '') {
		...
		$whereResult 	= '';
		$whereAll 		= 'trans_waktu_finish != "00:00:00" AND trans_id_loket != ""';
	...
	private function getChartData($trans_id_user, $trans_tanggal_transaksi) {
		...
		$this->db->where('trans_waktu_finish !=', '00:00:00' );
		$this->db->where('trans_id_loket !=', '' );
		//$this->db->where('a.id_user !=', '' );

		$grpx = 'admusr_username as info,';

		$this->db->Select($grpx . ",trans_tanggal_transaksi,COUNT(trans_id_layanan) as jumlah_customer, SEC_TO_TIME(AVG(TIME_TO_SEC(IF(TIMEDIFF(  trans_waktu_finish,  trans_waktu_panggil ) > 0, TIMEDIFF(  trans_waktu_finish,  trans_waktu_panggil ), 0)))) as rata_rata, DATE_FORMAT(trans_tanggal_transaksi, '%d-%m-%Y') as tgl_transaksi", FALSE);
	...
	function page_export_text() {
		$this->db->select('trans_tanggal_transaksi, DATE_FORMAT(trans_tanggal_transaksi, "%d-%m-%Y") as own_tanggal, SEC_TO_TIME(AVG(TIME_TO_SEC(IF(TIMEDIFF(  trans_waktu_finish,  trans_waktu_panggil ) > 0, TIMEDIFF(  trans_waktu_finish,  trans_waktu_panggil ), 0)))) as rata_rata', false);
	...
	function page_export_excel() {
		$filename = 'antrian_wl_summary_' . str_replace('-', '', date('Y-m-d')) . '.xls';
