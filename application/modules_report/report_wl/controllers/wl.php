...
class Wl extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'report_wl/wl/';
	private $_table_name 		= 'transaksi_log';
	private $_table_field_pref 	= 'trans_';
	private $_table_pk 			= 'trans_id_transaksi';
	private $_model_crud 		= 'reportwl_model';

	private $_page_title 		= 'Antrian : Admin Laporan Waktu Layanan';
	private $_page_content_info	= array(
		'title' => 'Data Admin Waktu Layanan',
		'desc' 	=> 'List laporan waktu layanan',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Wkt Layanan',
			...
			array(
				'title_header_column' 	=> 'Wkt Selesai',
	...
	function page_export_text() {
		...
		$this->db->select('trans_waktu_panggil, trans_waktu_finish, DATE_FORMAT(trans_tanggal_transaksi, "%d-%m-%Y") as own_tanggal, CONCAT(trans_no_ticket_awal, "", trans_no_ticket) as no_ticket, "-" as waktu_layanan', false);
	...
	function page_export_excel() {
		$filename = 'antrian_wl_' . str_replace('-', '', date('Y-m-d')) . '.xls';
		...
		$this->template->set('title', 'Antrian : Export Text WL');
		$this->template->set('assets', $this->_data['assets']);
