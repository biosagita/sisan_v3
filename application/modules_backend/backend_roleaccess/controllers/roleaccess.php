...
class Roleaccess extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_roleaccess/roleaccess/';
	private $_table_name 		= 'roleaccess';
	private $_table_field_pref 	= 'rolacc_';
	private $_table_pk 			= 'rolacc_id';
	private $_model_crud 		= 'roleaccess_model';

	private $_page_title 		= 'Antrian : Admin Role Access';
	private $_page_content_info	= array(
		'title' => 'Data Role Access',
		'desc' 	=> 'List role access',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Application Module',
				'field_name' 			=> 'appmod_name',
				'no_order'				=> 2,
			),
			array(
	...
	private function get_input_field_new($data_value = array()) {
		...
		$this->load->model('applicationmodules_model','applicationmodulesx');
		$source_application_module = $this->applicationmodulesx->get_option();
		...
		$data_input = array(
			...
			array(
				...
				'db_field' 		=> $this->_table_field_pref . 'appmod_id',
				'db_process'	=> true,
				'input_type'	=> 'select',
				'input_attr'	=> 'class="form-control"',
				'required'		=> 'required',
				'data_source'	=> $source_application_module,
				'data_edit'		=> array(
					'required'			=> 'required',
					'db_process'		=> true,
				),
	...
	function lists_ajax() {
		...
		$table 		.= ' LEFT JOIN ' . $this->db->dbprefix . 'adminuserlevel ON (rolacc_aulv_id = aulv_id) ';
		$table 		.= ' LEFT JOIN ' . $this->db->dbprefix . 'applicationmodules ON (rolacc_appmod_id = appmod_id) ';
