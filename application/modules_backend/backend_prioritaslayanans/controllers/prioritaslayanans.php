...
class Prioritaslayanans extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_prioritaslayanans/prioritaslayanans/';
	private $_table_name 		= 'prioritas_layanan';
	private $_table_field_pref 	= 'prilay_';
	private $_table_pk 			= 'prilay_id_prioritas';
	private $_model_crud 		= 'prioritaslayanans_model';

	private $_page_title 		= 'Antrian : Admin Prioritas Layanan';
	private $_page_content_info	= array(
		'title' => 'Data Prioritas Layanan',
		'desc' 	=> 'List Prioritas Layanan',
	);
	...
	private function get_show_column() {
		$column_list = array(
			array(
				...
				'field_name' 			=> $this->_table_field_pref . 'id_prioritas',
				'show_no_static' 		=> true,
				'no_order'				=> 0,
			),
			array(
				'title_header_column' 	=> 'Group Loket',
				'field_name' 			=> 'grolok_name',
				'no_order'				=> 1,
			),
			array(
			...
			array(
				'title_header_column' 	=> 'Prioritas',
				'field_name' 			=> $this->_table_field_pref . 'prioritas',
				'no_order'				=> 3,
			),
			array(
	...
	private function get_input_field_new($data_value = array()) {
		$this->load->model('grouplokets_model','grouplokets');
		$source_group_loket = $this->grouplokets->get_option();

		$this->load->model('grouplayanans_model','grouplayanans');
		$source_group_layanan = $this->grouplayanans->get_option();

		$data_input = array(
			array(
				'db_field' 		=> $this->_table_field_pref . 'id_prioritas',
				'db_pk' 		=> true,
				'db_process'	=> false,
				'data_edit'		=> array(
					'db_process'		=> false,
			...
			array(
				'label' 		=> 'Group Loket',
				'db_field' 		=> $this->_table_field_pref . 'id_group_loket',
				'db_process'	=> true,
				'input_type'	=> 'select',
				'input_attr'	=> 'class="form-control"',
				'required'		=> 'required',
				'data_source'	=> $source_group_loket,
				'data_edit'		=> array(
					'db_process'	=> true,
					'required'		=> 'required',
				),
			...
			array(
				'label' 		=> 'Prioritas',
				'db_field' 		=> $this->_table_field_pref . 'prioritas',
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" class="form-control" placeholder="Prioritas..."',
				'required'		=> 'required',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'	=> true,
	...
	function lists_ajax() {
		...
		$table 		.= ' LEFT JOIN ' . $this->db->dbprefix . 'grouplokets ON (prilay_id_group_loket = grolok_id) ';
		$table 		.= ' LEFT JOIN ' . $this->db->dbprefix . 'group_layanan ON (prilay_id_group_layanan = grolay_id_group_layanan) ';
