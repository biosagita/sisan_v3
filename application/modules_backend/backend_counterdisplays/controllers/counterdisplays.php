...
class Counterdisplays extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_counterdisplays/counterdisplays/';
	private $_table_name 		= 'counter_display';
	private $_table_field_pref 	= 'coudis_';
	private $_table_pk 			= 'coudis_id_counter_display';
	private $_model_crud 		= 'counterdisplays_model';

	private $_page_title 		= 'Antrian : Admin Counter Display';
	private $_page_content_info	= array(
		'title' => 'Data Counter Display',
		'desc' 	=> 'List Counter Display',
	);
	...
	private function get_show_column() {
		$column_list = array(
			array(
				...
				'field_name' 			=> $this->_table_field_pref . 'id_counter_display',
				'show_no_static' 		=> true,
				'no_order'				=> 0,
			),
			array(
				'title_header_column' 	=> 'Address CD',
				'field_name' 			=> $this->_table_field_pref . 'address_cd',
				'no_order'				=> 1,
			),
			array(
	...
	private function get_input_field_new($data_value = array()) {
		$this->load->model('lokets_model','lokets');
		$source_loket = $this->lokets->get_option();

		$data_input = array(
			array(
				'db_field' 		=> $this->_table_field_pref . 'id_counter_display',
				'db_pk' 		=> true,
				'db_process'	=> false,
				'data_edit'		=> array(
					'db_process'		=> false,
			...
			array(
				'label' 		=> 'Address CD',
				'db_field' 		=> $this->_table_field_pref . 'address_cd',
				'db_process'	=> true,
				'input_type'	=> 'text',
				'input_attr'	=> 'type="text" data-parsley-minlength="1" class="form-control" placeholder="Address CD..."',
				'required'		=> 'required',
				'data_source'	=> '',
				'data_edit'		=> array(
					'db_process'	=> true,
	...
	function lists_ajax() {
		...
		$table 		.= ' LEFT JOIN ' . $this->db->dbprefix . 'lokets ON (coudis_id_loket = lokets_id) ';
