...
class Usergrouplayanans extends MY_Admin {
	private $_template 			= 'template_admin/main';
	private $_module_controller = 'backend_usergrouplayanans/usergrouplayanans/';
	private $_table_name 		= 'user_group_layanan';
	private $_table_field_pref 	= 'usrgrolay_';
	private $_table_pk 			= 'usrgrolay_id';
	private $_model_crud 		= 'usergrouplayanans_model';

	private $_page_title 		= 'Antrian : Admin User Group Layanan';
	private $_page_content_info	= array(
		'title' => 'Data User Group Layanan',
		'desc' 	=> 'List Loket',
	);
	...
	private function get_show_column() {
		$column_list = array(
			...
			array(
				'title_header_column' 	=> 'Nama User Group',
				'field_name' 			=> 'usrgro_nama_user_group',
				'no_order'				=> 1,
			),
			array(
	...
	private function get_input_field_new($data_value = array()) {
		$this->load->model('usergroups_model','usergroups');
		$source_user_group = $this->usergroups->get_option();
		...
		$data_input = array(
			...
			array(
				'label' 		=> 'User Group',
				'db_field' 		=> $this->_table_field_pref . 'usrgro_id',
				'db_process'	=> true,
				'input_type'	=> 'select',
				'input_attr'	=> 'class="form-control"',
				'required'		=> 'required',
				'data_source'	=> $source_user_group,
				'data_edit'		=> array(
					'required'			=> 'required',
					'db_process'		=> true,
				),
			...
			array(
				'label' 		=> 'Group Layanan',
				'db_field' 		=> $this->_table_field_pref . 'grolay_id',
				'db_process'	=> true,
				'input_type'	=> 'select',
				'input_attr'	=> 'class="form-control"',
				'required'		=> 'required',
	...
	function lists_ajax() {
		...
		$table 		.= ' LEFT JOIN ' . $this->db->dbprefix . 'group_layanan ON (usrgrolay_grolay_id = grolay_id_group_layanan) ';
		$table 		.= ' LEFT JOIN ' . $this->db->dbprefix . 'user_group ON (usrgrolay_usrgro_id = usrgro_id) ';
